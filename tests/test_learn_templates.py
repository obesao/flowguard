"""Testa _cmd_learn_templates — deriva ddos_bps_threshold do baseline EWMA já
acumulado (prefix_baseline) e agrupa prefixos parecidos num template reutilizável,
em vez de copiar o mesmo número pra todo prefixo (achado real 2026-07-10: 30 Gbps
repetido em 7 dos 8 prefixos, com tráfego real variando de ~0.02 a ~10 Gbps)."""
...
def _insert_baseline(conn, dst_prefix, bps_mean, bps_std, samples, pps_mean=0, pps_var=0):
    conn.execute(
        """INSERT INTO prefix_baseline (dst_prefix, bps_mean, bps_var, pps_mean, pps_var, samples, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, 0)""",
        (dst_prefix, bps_mean, bps_std ** 2, pps_mean, pps_var, samples),
    )
    conn.commit()
