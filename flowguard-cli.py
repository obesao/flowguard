...
def cmd_status(args: argparse.Namespace, sock_path: str) -> None:
    ...
    table.add_row("Tráfego agregado (entrada)", fmt_bps(resp["bps"]))
    table.add_row("Tráfego agregado (saída)", fmt_bps(resp.get("bps_out", 0)))
    table.add_row("Pacotes/s (entrada)", f"{resp['pps']:,}".replace(",", "."))
    table.add_row("Pacotes/s (saída)", f"{resp.get('pps_out', 0):,}".replace(",", "."))
